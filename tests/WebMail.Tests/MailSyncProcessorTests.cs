...
public sealed class MailSyncProcessorTests
{
    ...
    [Fact]
    public async Task ProcessPendingFetchesAndStoresMessagesEvenWithNoAllowedSenders()
    {
        await using var db = CreateDb();
        SeedAccountWithoutAllowedSenders(db, buyerId: 1, accountId: 1, provider: "Fake");
        db.SyncJobs.Add(new SyncJob { BuyerId = 1, Status = SyncJobStatus.Pending });
        await db.SaveChangesAsync();
        ...
        Assert.Equal(SyncJobStatus.Succeeded, (await db.SyncJobs.SingleAsync()).Status);
        var stored = Assert.Single(await db.EmailMessages.ToListAsync());
        Assert.Equal("m-1", stored.ProviderMessageId);
    }
    ...
    [Fact]
    public async Task ProcessPendingDecryptsRefreshTokenBeforeFetch()
    {
        await using var db = CreateDb();
        db.AllowedSenders.Add(new AllowedSender { EmailAddress = "orders@example.com" });
        db.EmailAccounts.Add(new EmailAccount
        {
            Id = 1, BuyerId = 1, Email = "buyer@example.com",
            Provider = "Recording", ProviderUserId = "p", EncryptedRefreshToken = "enc:secret"
        });
        db.SyncJobs.Add(new SyncJob { BuyerId = 1, Status = SyncJobStatus.Pending });
        await db.SaveChangesAsync();

        var recorder = new RecordingProvider("Recording");
        await CreateProcessor(recorder).ProcessPendingAsync(db, DateTimeOffset.UtcNow, CancellationToken.None);

        Assert.Equal("secret", recorder.LastRefreshToken);
    }
    ...
    private static MailSyncProcessor CreateProcessor(IEmailProvider provider)
    {
        ...
        var config = new ConfigurationBuilder()
            ...
            .Build();
        return new MailSyncProcessor(resolver, config, new FakeTokenProtector());
    }
    ...
    private static void SeedBuyer(WebMailDbContext db, long id, EmailAuthorizationStatus email, ReviewStatus review = ReviewStatus.Approved, SupplierProcessingStatus supplier = SupplierProcessingStatus.Failed) =>
        db.Buyers.Add(new Buyer { Id = id, CardNo = $"card-{id}", EmailStatus = email, Stage = BuyerStage.Submitted, ReviewStatus = review, SupplierStatus = supplier });
    ...
    private sealed class RecordingProvider(string name) : IEmailProvider
    {
        public string Name { get; } = name;
        public string? LastRefreshToken { get; private set; }
        public OAuthStartResult BuildAuthorizationUrl(string cardNo, string redirectUri) => new(cardNo, cardNo);
        public Task<OAuthCallbackResult> CompleteAuthorizationAsync(string code, string state, string redirectUri, CancellationToken cancellationToken) => throw new NotImplementedException();
        public Task<IReadOnlyList<ProviderMessage>> FetchMessagesAsync(string refreshToken, IReadOnlyCollection<string> allowedSenders, DateTimeOffset? since, CancellationToken cancellationToken)
        {
            LastRefreshToken = refreshToken;
            return Task.FromResult<IReadOnlyList<ProviderMessage>>([]);
        }
    }
}
